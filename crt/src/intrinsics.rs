...
//! the job of compiler_builtins / libc / the unwinder in a hosted build, not std.
//!
...
//! `bcmp`, `strlen`, the aarch64 `getauxval`, and the unwind symbols
//! `rust_eh_personality` / `_Unwind_Resume`. We provide them all here so the
//! binary links with zero external dependencies.
//!
...
extern "C" fn _Unwind_Resume() -> ! {
    // Reaching here means unwinding is actually happening, which must not occur
    // under panic = "abort". Trap rather than depend on the std layer for exit.
